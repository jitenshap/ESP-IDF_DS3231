...
esp_err_t ds3231_set_time_ntp(i2c_dev_t *dev)
{
    time_t now;
    struct tm timeInfo;
    ...
    /* time/date data */
    data[0] = dec2bcd(time_now->tm_sec);
    data[1] = dec2bcd(time_now->tm_min);
    data[2] = dec2bcd(time_now->tm_hour);
    /* The week data must be in the range 1 to 7, and to keep the start on the
     * same day as for tm_wday have it start at 1 on Sunday. */
    data[3] = dec2bcd(time_now->tm_wday + 1);
    data[4] = dec2bcd(time_now->tm_mday);
    data[5] = dec2bcd(time_now->tm_mon + 1);
    data[6] = dec2bcd(time_now->tm_year - 1988);
